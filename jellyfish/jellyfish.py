...
warnings.warn('jellyfish changing the recursion limit')
import sys
...
class Jellyfish(Model):
    ...
    def __init__(self,
                    ...
                    init_bias_hid_h,
                    init_B, min_B, max_B,
                    init_alpha, min_alpha, max_alpha,
                    init_mu,
                       e_step,
                       m_step,
                    tied_B = False,
        ...
        irange: (scalar) g-h weights are initinialized ~U( [-irange,irange] )
        init_bias_hid_g, init_bias_hid_h: initial value of hidden biases (scalar or vector)
        ...
        if random_patches_src is not None:
            self.init_W = random_patches_src.get_batch_design(Nh).T
        else:
            self.init_W = None
        ...
        self.Ng = Ng
        self.irange = irange
        self.init_bias_hid_g = init_bias_hid_g
        self.init_bias_hid_h = init_bias_hid_h
    ...
    def redo_everything(self):
        ...
        U = self.rng.uniform(-self.irange, self.irange, (self.Nh, self.Ng) )

        self.U = sharedX(U, name = 'U')

        if self.init_W is not None:
            W = self.init_W.copy()
        else:
            W = self.rng.uniform(-1., 1., (self.Nv, self.Nh))
        ...
        self.bias_hid_g = sharedX(np.zeros(self.Ng)+self.init_bias_hid_g, name='bias_hid')
        self.bias_hid_g = sharedX(np.zeros(self.Nh)+self.init_bias_hid_h, name='bias_hid')
        self.alpha = sharedX(np.zeros(self.Nh)+self.init_alpha, name = 'alpha')
        self.mu = sharedX(np.zeros(self.Nh)+self.init_mu, name='mu')
        if self.tied_B:
            self.B_driver = sharedX(0.0+self.init_B, name='B')
        else:
            self.B_driver = sharedX(np.zeros(self.Nv)+self.init_B, name='B')

        self.test_batch_size = 2

        if self.recycle_q:
            self.prev_H = sharedX(np.zeros((self.recycle_q,self.Nh)), name="prev_H")
            self.prev_Mu1 = sharedX(np.zeros((self.recycle_q,self.Nh)), name="prev_Mu1")
    ...
    def em_functional(self, H, sigma0, Sigma1, stats):
        TODO-- HERE. does it really still make sense to talk EM functional?
