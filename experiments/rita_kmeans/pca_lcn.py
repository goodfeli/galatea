...
def store_PCA_rita(N_COMP,prefix,model,whiten=True):
    '''
    ...
    '''
    #Check if PCA with n-comp is already done
    if model is not None:
        print 'modele deja entraine'
    ...
    cPickle.dump(valid, open(prefix+'valid_postPCA_' + str(N_COMP) + 'comp.pkl','w'),-1)
    cPickle.dump(test,  open(prefix+'test_postPCA_' + str(N_COMP) + 'comp.pkl','w'),-1)
    ...
    cPickle.dump(train,open(prefix+'train_postPCA_' + str(N_COMP) + 'comp.pkl','w'),-1)
