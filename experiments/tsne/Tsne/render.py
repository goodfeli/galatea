...
def render(points, filename, width=3000, height=1800, fontfile=DEFAULT_FONT, fontsize=12, margin=0.05, transparency=0.5,label=None):
    """
    ...
    alpha = Image.new("RGB", im.size, (0,0,0))
    imtext = Image.new("RGB", im.size, color=(255,255,255))
    ...
    # Make a grayscale image of the font, white on black.
        pos = (x, y)
        #imtext = Image.new("RGB", im.size, (1,1,1))
        #drtext = ImageDraw.Draw(imtext)
        ...
        if fontfile is not None:
            if label == None:
                ...
                if label[colorcount]=="#000":
                    tmpcolor = "#111"
                else:
                    tmpcolor = label[colorcount]
                drtext.text(pos, title, font=font, fill=tmpcolor)
                colorcount += 1
        else:
            if label == None:
                ...
                if label[colorcount]=="#000":
                    ...
                    tmpcolor = label[colorcount]
                drtext.text(pos, title, fill=tmpcolor)
                colorcount += 1
