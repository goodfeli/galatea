...
class DSI_Softmax(Layer):
    ...
    def get_monitoring_channels(self):
        ...
        return OrderedDict([
                            ...
                            ('col_norms_mean' , col_norms.mean()),
                            ('col_norms_max'  , col_norms.max()),
                            ('slope_abs_min', abs(self.slope).min()),
                            ('slope_abs_mean', abs(self.slope).mean()),
                            ('slope_abs_max', abs(self.slope).max())
                            ])
    ...
    def censor_updates(self, updates):
        ...
        if not hasattr(self, 'max_abs_slope'):
            self.max_abs_slope = None

        if self.max_abs_slope is not None:
            if self.slope in updates:
                updates[self.slope] = T.clip(updates[self.slope], -self.max_abs_slope, self.max_abs_slope)


class DSI_Maxout(Layer):
    """
    A hidden layer that does max pooling over groups of linear
    units. If you use this code in a research project, please
    cite
    ...
    def set_input_space(self, space):
        ...
        self.slope = sharedX(rng.uniform(-self.islope, self.islope, (self.detector_layer_dim,)), 'slope')
    ...
    def censor_updates(self, updates):
        ...
        assert self.max_col_norm is None
    ...
    def fprop(self, state_below):
        ...
        z = self.transformer.lmul(state_below) * self.slope + self.b
