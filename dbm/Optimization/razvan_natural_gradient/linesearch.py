...
one = TT.constant(numpy.float32(1.))
zero = TT.constant(numpy.float32(0.))
...
def scalar_armijo_search(phi, phi0, derphi0, c1=numpy.float32(1e-4),
    ...
    def armijo(alpha0, alpha1, phi_a0, phi_a1):
        ...
        alpha2 = TT.switch(
            TT.bitwise_or(alpha1-alpha2 > alpha1 / numpy.float32(2.),
                  one - alpha2/alpha1 < 0.96),
            alpha1 / numpy.float32(2.),
            alpha2)
        return [alpha1, alpha2, phi_a1, phi_a2], theano.scan_module.until(end_condition)
...
def _cubicmin(a,fa,fpa,b,fb,c,fc):
    ...
    cond = lazy_or('cubicmin',radical < zero,
                         TT.eq(db , zero),
                         TT.eq(dc , zero),
                         TT.eq(b , c),
                         TT.eq(A , zero))
    ...
    xmin = TT.switch(cond, TT.constant(numpy.float32(numpy.nan)),
...
def test():
    ...
    W = theano.shared(numpy.asarray(
            numpy.random.uniform(size=(784,500))*1e-1,
            dtype = 'float32'), name ='W')
    v = theano.shared(numpy.asarray(
            numpy.random.uniform(size=(40,784))*1e-1,
            dtype = 'float32'), name = 'v')
    y = theano.shared(numpy.asarray(
        numpy.random.uniform(size=(40,500))*1e-1,
        dtype='float32'), name='y')
    ...
    grad = theano.function([x],
                           ...
                           mode=theano.Mode(linker='cvm'),
                           on_unused_input='warn')
    phi0 = theano.shared(numpy.asarray(func(0), dtype='float32'),
                               name = 'phi0')
    derphi0 = theano.shared(numpy.asarray(grad(0), dtype='float32'),
                                  name = 'derphi0')

    #outs = scalar_armijo_search(phi, phi0, derphi0, profile = 1)
    #
    #f = theano.function([], outs, profile = 1,
    #                   name = 'test_scalar_search',
    #                   mode = theano.Mode(linker='cvm'),on_unused_input='warn')

    rvals = scalar_search_wolfe2(phi, derphi,
                                 phi0, derphi0,
                                profile=1)
    f2 = theano.function([], rvals, profile = 1,
                         name='test_wolfe',
                         mode=theano.Mode(linker='cvm'),
                         on_unused_input='warn')
