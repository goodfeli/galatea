...
dummy_Y = sharedX(np.zeros((batch_size, model.hidden_layers[-1].n_classes))) + T.nnet.softmax(model.hidden_layers[-1].b)
#drop_mask = sharedX(rng.randint(0,2,(batch_size,784)).astype('float32'))
drop_mask = sharedX(np.zeros((batch_size,784)).astype('float32'))
drop_mask_Y = sharedX(np.ones(batch_size,))

_, ymf = model.do_inpainting(V=Xb, Y=dummy_Y, drop_mask=drop_mask, drop_mask_Y=drop_mask_Y, noise=False)
ymf.name = 'ymf'
assert Xb in theano.gof.graph.ancestors([ymf])
