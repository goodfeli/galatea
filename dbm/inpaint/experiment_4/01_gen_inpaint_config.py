...
copies = 1 + uniform_between(0., 1.) > .9
drop_prob = uniform_between(.01, .2)
duplicate = rng.randint(1,3,(num_jobs,))
activate = rng.randint(1,10, (num_jobs,))
layer_2_range_rewards = (uniform_between(0.,1.) > .8 ) * ( 10 ** uniform_between(-7., -1.))
...
layer_1_coeff = uniform_between(.01, .05)
layer_2_coeff = uniform_between(.0001, .03)
layer_2_coeff *= use_sparsity_2

# Layer 1
layer_1_dim = rng.randint(500,600, (num_jobs,))
...
# Layer 2
layer_2_dim = rng.randint(900,1100, (num_jobs,))
...
# Weight decay
wd_before = uniform_between(4e-7, 8e-7)
wd_after = 10 ** uniform_between(-7,-4)
