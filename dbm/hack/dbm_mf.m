...
for epoch = epoch:maxepoch 
  ...
  for batch_rr = rr; %1:numbatches
    ...
    data = double(data > 0.5);%rand(numcases,numdim));  
   [poshidprobs, pospenprobs, mean_range] = ...
         mf(data,targets,vishid,hidbiases,visbiases,hidpen,penbiases,labpen,hidrecbiases);
    mean_mean_range = mean_mean_range + mean_range;
  ...
  mean_mean_range = mean_mean_range / numbatches;

  fprintf(1, 'before epoch, recons misclass rate is %d\n', (60000-counter)/60000);
  fprintf(1, 'mean mean range is %d\n',mean_mean_range);

  fprintf(1,'epoch %d \t eps %d\r',epoch,epsilonw); 
  errsum=0;
...
%%%%% START NEGATIVE PHASE  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  % Ian's modification
  negprodspen = neghidprobs'*negpenstates;
...
%%%%%%%%%%%%%%% END OF UPDATES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
  ...
  fprintf(1, 'epoch %4i reconstruction error %6.1f \n Proportion misclassified training cases %d (out of 60000) \n', epoch, errsum, (60000 -counter) / 60000. ); 
