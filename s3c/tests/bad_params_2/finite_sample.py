...
from galatea.s3c.s3c import DebugEnergy
...
class TestWithFiniteSamples:
    ...
    def test_entropy_hs(self):
        ...
        for i in xrange(num_samples):
            ...
            warnings.warn('this is not numerically stable')
    ...
    def test_expected_energy_vhs(self):
        ...
        assert H.shape == (1,self.N)

        Mu1 = np.cast[config.floatX](rng.uniform(-5.0,5.0,(self.m,self.N)))
        assert Mu1.shape == (1, self.N)
        ...
        debug = DebugEnergy(v_term = False)

        analytical_energy = model.expected_energy_vhs(V, H_var, mu0, Mu1_var, sigma0, Sigma1, debug)
        ...
        energy = model.energy_vhs(V, H_sample, final_sample, debug)
        ...
        for i in xrange(num_samples):
            ...
            if i % write_freq == 0:
                x[i/write_freq] = i
                diffs[i/write_freq] = np.abs(approx_energy - analytical_energy)
