...
def validate(train_X, y_fine, y_coarse, fold_indices, omnivore_classifier,
        fruit_classifier, aux_X, aux_y, C, one_against_many):

    train_mask = np.zeros((train_X.shape[0],),dtype='uint8')
    train_mask[fold_indices-1] = 1

    X = np.concatenate( (train_X[train_mask.astype(bool),:], aux_X), axis=0)
    y = np.concatenate( (y_coarse[train_mask.astype(bool)], aux_y), axis=0)
...
def train(fold_indices, omnivore_classifiers, fruit_classifiers, train_X, y_fine, y_coarse,
        aux_X, aux_y, report, C_list, one_against_many=False, skip_cv = False ):
    """
        ...
        for C in param_grid['C']:
            ...
            for i in xrange(fold_indices.shape[0]):
                ...
                this_fold_acc = validate(train_X, y_fine, y_coarse, fold_indices[i,:],
                        omnivore_classifiers[i], fruit_classifiers[i], aux_X, aux_y, C, one_against_many)
