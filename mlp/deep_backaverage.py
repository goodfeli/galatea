...
from galatea.mlp import ConvLinearAveC01B
...
def add_filters_and_act_record(layer):
    if 'CompositeLayer' in str(type(layer)):
        ...
        elif isinstance(layer, (ConvLinearC01B, ConvLinearAveC01B)):
            num_filters.append(layer.output_space.num_channels)
        else:
...
topo = X.ndim != 2
H_hat = model.fprop(X, apply_dropout=False, return_all=True)
acts = []
...
def add_acts(layer, state):
    ...
    if isinstance(layer, super_dbm.CompositeLayer):
        ...
        shape = layer.get_output_space().shape
        i = shape[0] / 2
        j = shape[1] / 2
        acts.append(state[:,i,j,:].T)
    else:
        p = state
        acts.append(p)


for layer, state in zip(model.layers[:-1], H_hat):
...
for layer_idx in xrange(num_layers):
    ...
    for filter_idx in xrange(num_filters[layer_idx]):
        ...
        # regard negative activations as very inhibitory, don't let them count for any response
        filter_act_record *= filter_act_record > 0.
        ...
        #    num_top_filters += 1
        num_top_filters = int(0.01 * len(filter_act_record))
        ...
        #if hasattr(dataset, 'mapback_for_viewer'):
        #    blah = dataset.get_design_matrix(img)
        #    blah = dataset.mapback_for_viewer(blah)
        #    img = dataset.get_topological_view(blah)
        #else:
        img /= np.abs(img).max()
